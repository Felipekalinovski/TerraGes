...
import {formatQueryResponse,handleTerragesQueryTurn,inferDeterministicQuery,isTerragesQueryCandidate} from '../../supabase/functions/_shared/whatsapp-query-engine.ts';
...
test('common operational queries are typed locally without an LLM call',()=>{
 assert.deepEqual(inferDeterministicQuery('Quais OS abertas existem?')?.intent,'open_service_orders');
 assert.deepEqual(inferDeterministicQuery('Qual é o status da CAT 320?')?.machine_query,'CAT 320');
 assert.deepEqual(inferDeterministicQuery('Resumo financeiro deste mês')?.intent,'financial_summary');
});

test('financial query goes through the scoped database RPC and is delivered once',async()=>{
 let saved='',modelCalls=0;const calls:any[]=[];
 ...
 const fetcher=async(url:any)=>{if(String(url).includes('openrouter')){modelCalls++;return new Response('{}');}return new Response(JSON.stringify({key:{id:'provider-1'}}));};
 ...
 const query=calls.find(x=>x.name==='run_whatsapp_operational_query');assert.equal(query.args.p_limit,10);assert.equal('company_id' in query.args,false);assert.match(saved,/Saldo/);assert.equal(modelCalls,0);
});
