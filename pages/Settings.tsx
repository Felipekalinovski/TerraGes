    ...
    <Layout title="Configurações" hideNav={false}>
      <div className="p-4 space-y-3 pt-6">
        {menuItems.map((item, idx) => (
          <button
            ...
            className="w-full flex items-center justify-between p-4 rounded-xl bg-surface-dark border border-white/5 hover:border-primary/30 transition-all group"
          >
            <div className="flex items-center gap-4">
              <div className="p-2 rounded-lg bg-[#4b3220] text-text-gold group-hover:text-primary transition-colors">
                {item.icon}
              </div>
      ...
      <div className="px-4 mt-8">
        <button
          ...
          className="w-full p-4 rounded-xl bg-surface-dark border border-red-500/20 text-red-400 font-medium hover:bg-red-500/10 transition-colors flex items-center justify-center gap-2"
        >
          <LogOut size={20} />
          Sair da Conta
        </button>
        <p className="text-center text-xs text-gray-600 mt-4">Versão 1.0.2 • TerraGes</p>
      </div>
    </Layout>
  );
};
