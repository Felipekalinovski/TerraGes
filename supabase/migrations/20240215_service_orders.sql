...
-- Seed data for employees (using WHERE NOT EXISTS to avoid duplicates if run multiple times)
INSERT INTO employees (name, role, status, contact, image_url)
SELECT 'Carlos Silva', 'Operador de Máquinas', 'active', '(11) 99999-1111', 'https://i.pravatar.cc/150?u=1'
WHERE NOT EXISTS (SELECT 1 FROM employees WHERE name = 'Carlos Silva');

INSERT INTO employees (name, role, status, contact, image_url)
SELECT 'Ana Pereira', 'Engenheira Civil', 'active', '(11) 98888-2222', 'https://i.pravatar.cc/150?u=5'
WHERE NOT EXISTS (SELECT 1 FROM employees WHERE name = 'Ana Pereira');

INSERT INTO employees (name, role, status, contact, image_url)
SELECT 'João Costa', 'Mecânico Chefe', 'vacation', '(11) 97777-3333', 'https://i.pravatar.cc/150?u=3'
WHERE NOT EXISTS (SELECT 1 FROM employees WHERE name = 'João Costa');

INSERT INTO employees (name, role, status, contact, image_url)
SELECT 'Mariana Souza', 'Assistente Adm.', 'active', '(11) 96666-4444', 'https://i.pravatar.cc/150?u=9'
WHERE NOT EXISTS (SELECT 1 FROM employees WHERE name = 'Mariana Souza');

INSERT INTO employees (name, role, status, contact, image_url)
SELECT 'Roberto Lima', 'Motorista', 'leave', '(11) 95555-5555', 'https://i.pravatar.cc/150?u=8'
