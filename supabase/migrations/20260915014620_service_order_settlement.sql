-- Reliable service-order completion; private ledger with tenant-scoped UI metadata.
...
-- Expose only source/transaction IDs to the current company's managers.
CREATE INDEX service_order_settlements_company_idx ON private.service_order_settlements(company_id);
CREATE INDEX service_order_settlements_actor_idx ON private.service_order_settlements(completed_by);
CREATE FUNCTION private.list_service_order_settlements()
RETURNS TABLE(service_order_id uuid, transaction_id uuid)
LANGUAGE sql STABLE SECURITY DEFINER SET search_path='' AS $$
 SELECT s.service_order_id,s.transaction_id FROM private.service_order_settlements s
 WHERE auth.uid() IS NOT NULL AND private.is_manager() AND s.company_id=private.current_company()
$$;
REVOKE ALL ON FUNCTION private.list_service_order_settlements() FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION private.list_service_order_settlements() TO authenticated;
CREATE FUNCTION public.get_service_order_settlements()
RETURNS TABLE(service_order_id uuid, transaction_id uuid)
LANGUAGE sql STABLE SECURITY INVOKER SET search_path='' AS $$
 SELECT * FROM private.list_service_order_settlements()
$$;
REVOKE ALL ON FUNCTION public.get_service_order_settlements() FROM PUBLIC,anon,authenticated;
GRANT EXECUTE ON FUNCTION public.get_service_order_settlements() TO authenticated;
NOTIFY pgrst,'reload schema';
