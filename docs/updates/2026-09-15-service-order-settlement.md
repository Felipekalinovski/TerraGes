...
- Advisors antes da publicação: avisos existentes sobre schema pessoal, extensão vector, RPCs anteriores de vínculo WhatsApp e proteção de senhas. Nenhuma correção fora desta etapa foi aplicada.
...
Aplicar esta migração no Supabase antes do merge/deploy do frontend: a nova tela financeira depende da RPC. Após aplicar, repetir os testes SQL com rollback e verificar os advisors. Integrar o PR #3 na main e acompanhar o deploy automático no domínio de produção.
